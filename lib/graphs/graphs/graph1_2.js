let cy1_2 = cytoscape({
  container: document.getElementById('graph1-2-cy'),
...
cy1_2.on('mouseover', 'node', function(e){
  var sel = e.target;
  highlight(sel, cy1_2);
});
cy1_2.on('mouseout', 'node', function(e){
  dehighlight(cy1_2);
});

cy1_2.on('mouseover', 'edge', function(e){
  var sel = e.target;
  sel.addClass('highlight');
});
cy1_2.on('mouseout', 'edge', function(e){
  var sel = e.target;
  sel.removeClass('highlight');
});

var cytoEventHandler = function(event) {
  if (event.currentSlide.id === 'graph1_2') {
    for(let i = 0; i < 8; i++) {
      cy1_2.nodes()[i].animate({
        css: {
          'background-color': 'blue',
          'color': 'white'
          }
